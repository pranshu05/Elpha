...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const kick_db_fail = new Discord.MessageEmbed()
            ...
            .setDescription(
                `An error occurred while adding kicked user to database!`
            )
        ...
        if (
            ...
            if (user.id === '754381104034742415')
                return interaction.reply('You cannot kick my developer')
            if (user === interaction.user)
                return interaction.reply('You cannot kick yourself')
            if (user === interaction.client.user)
                return interaction.reply('You cannot kick me')
            if (
                interaction.guild.members.cache
                    .get(user.id)
                    .permissions.has(
                ...
                return interaction.reply(
                    'You cannot kick member with higher roles!'
                )
            }
            if (!kick_member.kickable)
                return interaction.reply(
                    'I cannot kick the member because they have roles above me or you!'
                )
            ...
            if (!modlog) {
                ...
                abc.send({ embeds: [kicked_embed] })
            }
            user.send(`You were kicked from ${interaction.guild.name}`).catch(
                console.error
            )
        } else {
