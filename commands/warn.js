...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('warning').setDescription('warning').setRequired(true)
    ),
  async execute(interaction) {
    ...
    if (
      interaction.guild.members.cache
        ...
        .permissions.has(Discord.Permissions.FLAGS.MANAGE_CHANNELS)
    ) {
      if (user.id === '754381104034742415') {
        return interaction.reply('You cannot warn my developer');
      }
      ...
      if (
        ...
        user.id === '754381104034742415'
      ) {
        return interaction.reply('You cannot warn member with higer roles!');
      }
      ...
      Warned.findOne({ guild_id: interaction.guild.id }, (err, settings) => {
        if (err) {
          console.log(err);
          interaction.reply(
            'An error occurred while adding warned user to database!'
          );
          return;
        } else {
          settings = new Warned({
      ...
      Warning.findOne(
        ...
        (err, settings) => {
          if (err) {
            console.log(err);
            interaction.reply(
              "An error occurred while adding warning to user's database!"
            );
            return;
          } else {
