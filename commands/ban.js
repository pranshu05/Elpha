...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const ban_db_fail = new Discord.MessageEmbed()
      ...
      .setDescription(
        `An error occurred while adding banned user to database!`
      );
    ...
    if (
      interaction.guild.members.cache
        ...
        .permissions.has(Discord.Permissions.FLAGS.BAN_MEMBERS)
    ) {
      if (user.id === '754381104034742415')
        return interaction.reply('**You cannot ban my developer**');
      if (user === interaction.user)
        return interaction.reply('**You cannot ban yourself**');
      if (user === interaction.client.user)
        return interaction.reply('**You cannot ban me**');
      if (user === interaction.guild.owner)
        return interaction.reply('**You cannot ban owner of this server!**');
      if (!member.bannable)
      ...
      if (
        ...
        return interaction.reply(
          '**You cannot ban member with higher roles!**'
        );
