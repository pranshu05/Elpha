...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if(interaction.guild.members.cache.get(interaction.user.id).permissions.has(Discord.Permissions.FLAGS.MANAGE_MESSAGES)){
            if(user === interaction.user) return interaction.reply('You cannot timeout yourself')
            if(user === interaction.client.user) return interaction.reply('You cannot timeout me')
            if(!ms(duration) || ms(duration) > ms('28d')) return interaction.reply('Provided duration is invalid or greater than 28days!')
            if(interaction.guild.members.cache.get(user.id).permissions.has(Discord.Permissions.FLAGS.MANAGE_MESSAGES) || interaction.guild.members.cache.get(user.id).permissions.has(Discord.Permissions.FLAGS.ADMINISTRATOR)){
                interaction.reply('You cannot timeout member with higher roles!')
                return 
            }
            if(user.id === '754381104034742415') return interaction.reply('You cannot timeout my developer')
            ...
            user.send(`You were timed out in ${interaction.guild.name} for duration: ${duration}`).catch(console.error)
        }else{
