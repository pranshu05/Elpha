...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('name').setDescription('name of the GIF').setRequired(true)
    )
    .addStringOption((option) =>
      option.setName('url').setDescription('URL of GIF').setRequired(true)
    ),
  async execute(interaction) {
    ...
    if (gif.length !== 0) {
      interaction.reply(
        `Gif named **${name}** already exist in this server! Please select another name.`
      );
      return;
    } else {
