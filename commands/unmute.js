...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const unmute_db_fail = new Discord.MessageEmbed()
            ...
            .setDescription(
                `An error occurred while removing muted user from database!`
            )
        ...
        if (
            ...
            if (!muteRole)
                return interaction.reply(
                    'User or this server donot contain a role named **Mute**'
                )
            ...
            user.send(`You were unmuted in ${interaction.guild.name}`).catch(
                console.error
            )
        } else {
