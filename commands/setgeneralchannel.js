...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const gen_embed = new Discord.MessageEmbed()
            .setColor('#00ff00')
            .setTitle(
                `**:white_check_mark: General channel has been set to ${interaction.options.getChannel(
                    'general'
                )}**`
            )
        ...
        if (
            !interaction.guild.me
                .permissionsIn(interaction.options.getChannel('general'))
        ...
        General.findOne({ guild_id: interaction.guild.id }, (err, settings) => {
            ...
            if (!settings) {
                settings = new General({
                    ...
                    general_channel_id:
                        interaction.options.getChannel('general').id,
                })
            } else {
